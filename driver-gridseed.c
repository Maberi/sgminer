...
static int gridseed_ftdi_init(struct cgpu_info *gridseed, int interface)
{
	...
	err = usb_transfer(gridseed, FTDI_TYPE_OUT, FTDI_REQUEST_DATA,
				FTDI_VALUE_DATA_GSD, interface, C_SETDATA);
	...
	// Set the baud
	err = usb_transfer(gridseed, FTDI_TYPE_OUT, FTDI_REQUEST_BAUD, FTDI_VALUE_BAUD_GSD,
				(FTDI_INDEX_BAUD_GSD & 0xff00) | interface,
				C_SETBAUD);
